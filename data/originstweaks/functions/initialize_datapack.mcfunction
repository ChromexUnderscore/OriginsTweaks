team add arachnid "Arachnid"
team add avian "Avian"
team add blazeborn "Blazeborn"
team add elytrian "Elytrian"
team add enderian "Enderian"
team add feline "Feline"
team add merling "Merling"
team add phantom "Phantom"
team add shulk "Shulk"
team modify arachnid color green
team modify avian color white
team modify blazeborn color dark_red
team modify elytrian color dark_gray
team modify enderian color dark_purple
team modify feline color yellow
team modify merling color aqua
team modify phantom color dark_aqua
team modify shulk color light_purple
team modify arachnid seeFriendlyInvisibles false
team modify avian seeFriendlyInvisibles false
team modify blazeborn seeFriendlyInvisibles false
team modify elytrian seeFriendlyInvisibles false
team modify enderian seeFriendlyInvisibles false
team modify feline seeFriendlyInvisibles false
team modify merling seeFriendlyInvisibles false
team modify phantom seeFriendlyInvisibles false
team modify shulk seeFriendlyInvisibles false
tellraw @a "OriginsTweaks 1.7.1 Initialized."